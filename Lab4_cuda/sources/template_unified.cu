...
int main(int argc, char **argv) {
  ...
  float *unifiedInput;
  float *unifiedOutput;
  int intputSize = numInputElements * sizeof(float);
  int outputSize = numOutputElements * sizeof(float);
  cudaMallocManaged((void **)&unifiedInput, intputSize);
  cudaMallocManaged((void **)&unifiedOutput, outputSize);
  ...
  total<<<dimGrid, dimBlock>>>(
    unifiedInput, unifiedOutput, numInputElements
  );
  ...
  cudaFree(unifiedInput);
  cudaFree(unifiedOutput);
